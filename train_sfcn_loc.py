...
# Load Location Maps
fi = open("location_maps/loc_map_x.p", 'rb')
loc_x = pickle.load(fi)
loc_x = loc_x.reshape(loc_x.shape + (1,))
fi = open("location_maps/loc_map_y.p", 'rb')
loc_y = pickle.load(fi)
loc_y = loc_y.reshape(loc_y.shape + (1,))
fi = open("location_maps/loc_map_x_mid.p", 'rb')
loc_x_mid = pickle.load(fi)
loc_x_mid = loc_x_mid.reshape(loc_x_mid.shape + (1,))
...
train_size = int(ratio*partition)
test_size = int(partition-ratio*partition)

loc_map = np.concatenate((loc_x, loc_y), axis = -1)
loc_map_train = np.zeros((train_size, 32, 32, 2))
loc_map_test = np.zeros((test_size, 32, 32, 2))

for i in range(train_size):
	loc_map_train[i,:,:,:] = loc_map

for i in range(test_size):
	loc_map_test[i,:,:,:] = loc_map
...
inputs_loc = Input(shape=loc_map_shape) # 32 x 32 x 2
...
concat_loc = concatenate([concat_pool4, inputs_loc], axis=-1) # 32 x 32 x (256 + #channels of location map) 
...
model_path = model_dir + 'sfcn_loc_model_{epoch:02d}_{loss:.4f}_{val_loss:.4f}.h5'
...
history = model.fit(x=[img_train, cnt_train, loc_map_train], y=y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=([img_val, cnt_val, loc_map_test], y_val), callbacks=[callbacks_weight, callbacks_model, AdamLearningRateTracker()])
...
with open('sfcn_loc_results/loss_history/sfcn_loc_history.pickle', 'wb') as file_pi:
